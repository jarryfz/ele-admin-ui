  ...
  methods: {
    ...
    updateWidth() {
      ...
      const bodyWrapperDom = this.parentDom[0].getElementsByClassName("el-table__body-wrapper")[0],
       width = getComputedStyle(bodyWrapperDom).width, // 表格宽度
      // 给表格设置宽度。
       tableParent = this.tableDom
    ...
    scrollEvent: rafThrottle(async function (e) {
      ...
      let {height, top, left} = this.tablexy,
       ...
       {fixedRightHeaderDom, fixedRightBox, fixedLeftHeaderDom, fixedLeftBox, dom} = this.getFixedDom()
    ...
    clearFixedStyle() {
      ...
      let {height, left} = this.tablexy,
       {dom, fixedRightHeaderDom, fixedRightBox, fixedLeftHeaderDom, fixedLeftBox} = this.getFixedDom()
      if (dom.classList.contains("fixed")) {
        if (fixedRightHeaderDom) {
